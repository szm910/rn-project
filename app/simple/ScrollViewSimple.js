...
import { AppRegistry, ScrollView, Image, Text,StyleSheet,View,Button,TextInput,ToastAndroid } from 'react-native'

export default class ScrollViewSimple extends Component {
        ...
        return (
            <View>
                ...
                <ScrollView
                    ...
                    <Text  style={{ height: 50, backgroundColor: 'black', color: 'black' }}>悬浮在顶部</Text>
                    <TextInput />
                    {
                        this.state.data.map((item, index) => {
                            var color = index * 23 + 10;
                            return <Text key={{index}} style={[styles.text, { backgroundColor: color }]}>{item}</Text>
                        })
                    }
                </ScrollView>
            </View>
